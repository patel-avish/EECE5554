...
def driver():
    ...
    while not rospy.is_shutdown():
        ...
        # Read only GPGGA data which corresponds to GNSS
        if "$GNGGA" in str(recieve):
            ...
            #time conversion
            UTC = float(data[1])
            time_data = str(data[1]).split(".")
            t = float(time_data[0])
            ns = float(time_data[1])
            
            utc_hrs = t//10000
            utc_min = (t - (utc_hrs*10000))//100
            utc_sec = (t - (utc_hrs*10000) - (utc_min*100))
            utc_final_secs = (utc_hrs*3600 + utc_min*60 + utc_sec)
            utc_final_nsecs = (ns* (10**6))
